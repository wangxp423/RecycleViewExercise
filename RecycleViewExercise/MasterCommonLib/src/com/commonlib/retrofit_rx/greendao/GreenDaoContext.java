...
/**
 * @类描述：重写Context主要用户配置db文件存储路径
 * @创建人：Wangxiaopan
 * @创建时间：2017/6/8 0008 18:32
 * @修改人：
 * @修改时间：2017/6/8 0008 18:32
 * @修改备注：
 */

public class GreenDaoContext extends ContextWrapper {

    private String currentUserId = "driver";//一般用来针对一个用户一个数据库，以免数据混乱问题
    private Context mContext;

    public GreenDaoContext() {
        super(RxRetrofitApp.getApplication());
        this.mContext = RxRetrofitApp.getApplication();
//        this.currentUserId = "greendao";//初始化
    ...
    public File getDatabasePath(String dbName) {
        String dbDir = FileUtil.getFilePath(mContext);
        if (TextUtils.isEmpty(dbDir)) {
            LogUtils.e("Test", "SD卡不存在，请加载SD卡");
            return null;
        }
