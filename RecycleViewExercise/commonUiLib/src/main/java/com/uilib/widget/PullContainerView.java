            ...
            for (int i = 0; i < N; i++) {
                ...
                if (attr == R.styleable.PullContainerView_enable) {
                    ...
                    if (viewLayout != View.NO_ID)
                        mHeaderView = inflater.inflate(viewLayout, this, false);
                } else if (attr == R.styleable.PullContainerView_footerView) {
                    final int viewLayout = typedArray.getResourceId(attr, View.NO_ID);
                    if (viewLayout != View.NO_ID)
                        mFooterView = inflater.inflate(viewLayout, this, false);
...
//              SDK Version: 9  Build.VERSION_CODES.GINGERBREAD (2.3)
    ...
    public void init() {
        ...
        if (mCoupleView != null) {
            ...
            mCoupleViewMarginTop = ((MarginLayoutParams) lp).topMargin;
            mCoupleViewMarginBottom = ((MarginLayoutParams) lp).bottomMargin;
            RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(lp);
            params.addRule(RelativeLayout.BELOW, mMarginView.getId());
...
//        System.out.println("setPullState: " + state + ";  orientation = " + orientation);
        ...
        if (orientation == ORIENTATION_DOWN) {
            ...
            if (mHeaderCallback != null)
                mHeaderCallback.onStateChanged(mHeaderView, state, oldState);
            if (mHeaderView != null) {
        ...
        if (orientation == ORIENTATION_UP) {
            ...
            if (mFooterCallback != null)
                mFooterCallback.onStateChanged(mFooterView, state, oldState);
            if (mFooterView != null) {
                if (state == PULL_STATE_NONE) {
...
//                    scrollTo(getScrollX(), getMainViewScrollY());  //最开始的写法
                    mScrollHelper.startUsingDistance(mHeaderView.getHeight(), ScrollHelper.ORIENTATION_VERTICAL, 500);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mPullDownState == PULL_STATE_LOADING || mPullUpState == PULL_STATE_LOADING)
                    return true;
                    ...
                    if (view != null) {
                        if (view instanceof AbsListView) {
                            ...
                            if (mPullUpState == PULL_STATE_NONE) {
                                ...
                                if (last == topView.getCount() - 1)
                                    bottomChild = topView.getChildAt(topView.getChildCount() - 1);
                ...
                if (mPullUpState == PULL_STATE_PULLING || mPullUpState == PULL_STATE_ENABLE) {
                    if (py <= mScrollingY) {
                        ...
                        if (mFooterCallback != null)
                            mFooterCallback.onHeightChanged(mFooterView, detal, mPullUpState);
                        return true;
                    } else {
                    ...
                    if (py >= mScrollingY) {
                        ...
                        if (mHeaderCallback != null)
                            mHeaderCallback.onHeightChanged(mHeaderView, detal, mPullDownState);
                        return true;
                    } else {
                        scrollTo(getScrollX(), getCoupleViewScrollY());
                        setPullState(PULL_STATE_NONE, ORIENTATION_DOWN);
